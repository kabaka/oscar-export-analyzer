...
- Apply this WebKit modal-dismissal workaround to any Playwright dialog/modal interaction (for example, the storage-consent `alertdialog`), not just one spec.
...
The wearable integration imports a **local Google Health (formerly Fitbit) export** — there is no OAuth, no API worker, and no token encryption to test. The earlier OAuth/API test suites and their helpers (`FitbitOAuth.e2e.test.jsx`, `oauthTestHelpers.js`, `fitbitApi.worker.js`, and similar) were removed along with that flow.

Test the ingestion and correlation pipeline through its **plain, colocated unit tests** rather than through a real Worker or network mocks:

- **Ingestion logic** — `src/utils/wearable/ingestEngine.test.js`, `src/utils/wearable/wearableNight.test.js`, and the parser/aggregator tests under `src/utils/wearable/`. All ingestion logic lives in plain modules so it is unit-testable without a real Worker.
- **Hooks** — `src/hooks/useWearableImport.test.js`, `src/hooks/useWearableData.test.js`, `src/hooks/useWearableCorrelation.test.js`.
- **Correlation math** — `src/utils/wearableCorrelation.test.js` and `src/utils/wearableAnalysis.test.js`. Verify FDR correction and effect-size outputs against known inputs, and cover the few-overlapping-nights and missing-metric cases.
- **Storage migration** — `src/utils/appDb.wearable.test.js` confirms the IndexedDB schema upgrade drops legacy stores while preserving `sessions`.
- **UI** — `src/components/wearable/WearableImportCard.test.jsx`, `IngestStatusPanel.test.jsx`, `WearableDashboard.test.jsx`, and `src/features/wearable-correlation/Section.test.jsx`. Cover the non-Chromium unsupported empty-state by stubbing `showDirectoryPicker` absence.

### Key Testing Patterns

**✅ DO:**

- Test ingestion logic directly against the plain `utils/wearable/*` modules — no real Worker needed.
- Use **synthetic export fixtures only** — a real export is PHI. Build small synthetic file trees that exercise the allowlist, schema drift, and night-keying edge cases.
- Cover the non-Chromium unsupported state by stubbing `showDirectoryPicker` absence.
- Verify correlation FDR correction and effect sizes against known inputs.

**❌ DON'T:**

- Use a real Google Health / Fitbit export in tests — it is real patient data.
- Reintroduce OAuth/token/network mocks; the integration makes no network request for health data.

See [Wearable Integration](wearable-integration.md#testing-patterns) for the full testing strategy and pipeline overview.
