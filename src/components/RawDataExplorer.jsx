...
function VirtualTable({ rows, columns, rowHeight = 28, height = 360, renderRow }) {
  ...
  return (
    <div
      ref={containerRef}
      onScroll={onScroll}
      style={{ position: 'relative', overflow: 'auto', height: `${height}px`, border: '1px solid #ddd' }}
    >
...
export default function RawDataExplorer({
  ...
  useEffect(() => setVisibleCols(allColumns), [allColumns.join('|')]);
  ...
  const exportSelection = () => {
    ...
    const selectedRows = Array.from(selected).sort((a,b)=>a-b).map(i => sorted[i]).filter(Boolean);
  ...
  const pivot = useMemo(() => {
    ...
    const rowsOut = Array.from(groups.entries()).map(([g, list]) => {
      ...
      nums.forEach(c => {
        const vals = list.map(r => r?.[c]).filter(v => typeof v === 'number');
        const avg = vals.length ? vals.reduce((a,b)=>a+b,0)/vals.length : null;
        o[`avg_${c}`] = avg;
      });
      return o;
    });
    return { columns: [pivotBy, 'count', ...nums.map(c => `avg_${c}`)], rows: rowsOut };
  }, [sorted, pivotBy]);
    ...
    <section>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        <div role="tablist" aria-label="Raw data tabs" style={{ display: 'inline-flex', gap: 4 }}>
          ...
          <button aria-selected={tab==='details'} onClick={() => setTab('details')} disabled={!detailRows?.length}>Details</button>
        </div>
        <input
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {allColumns.map(c => (
            <label key={c} style={{ minWidth: 160 }}>
              <input type="checkbox" checked={visibleCols.includes(c)} onChange={() => toggleCol(c)} /> {c}
            </label>
          ))}
        </div>
      </details>
      ...
      <div style={{ overflowX: 'auto', marginTop: 8 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            <tr style={{ height: 0 }}><td colSpan={(visibleCols.length ? visibleCols : allColumns).length + 1} style={{ padding: 0 }}>
              <VirtualTable
                rows={sorted}
                columns={visibleCols.length ? visibleCols : allColumns}
                rowHeight={28}
                height={360}
                renderRow={(row, idx) => (
                  <div role="row" key={idx} style={{ display: 'table', tableLayout: 'fixed', width: '100%' }}>
                    ...
                    {(visibleCols.length ? visibleCols : allColumns).map(c => (
                      <div role="cell" key={c} style={{ display: 'table-cell', padding: '4px 6px', borderBottom: '1px solid #eee' }}>
                        {String(row?.[c] ?? '')}
                      </div>
                    ))}
                  </div>
        ...
        <div style={{ overflowX: 'auto', marginTop: 8 }}>
          <table>
            ...
            <tbody>
              {pivot.rows.map((r, i) => (
                <tr key={i}>
                  {pivot.columns.map(c => <td key={c}>{typeof r[c] === 'number' ? r[c].toFixed(2) : String(r[c])}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
